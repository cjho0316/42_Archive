...
- env : 옵션이 따로 필요하지 않음 (어떻게 데이터를 관리할지 고민(연결리스트, 이중포인터))
...
- pwd : getcwd 명령어 사용해서 간단히 구현

- cd : 인자준다음에 없는 경로로 이동하려고 하면 'bash: cd: 인자: No such file or directory' 오류출력
